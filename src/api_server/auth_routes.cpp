...
void setupAuthRoutes(
    ...
    // ========================================================================
    server.Post("/api/v1/auth/register", [auth_manager](const httplib::Request& req, httplib::Response& res) {
        try {
    ...
    // ========================================================================
    server.Post("/api/v1/auth/login", [auth_manager](const httplib::Request& req, httplib::Response& res) {
        try {
            json body = json::parse(req.body);
    ...
    // ========================================================================
    server.Post("/api/v1/auth/refresh", [auth_manager](const httplib::Request& req, httplib::Response& res) {
        try {
            json body = json::parse(req.body);
    ...
    // ========================================================================
    server.Post("/api/v1/auth/logout", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
    ...
    // ========================================================================
    server.Get("/api/v1/auth/me", [auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require authentication
    ...
    // ========================================================================
    server.Post("/api/v1/auth/change-password", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require authentication
        auto user = auth_middleware->getRequestUser(req);
        if (!user) {
            sendError(res, 401, "Authentication required");
    ...
    // ========================================================================
    server.Post("/api/v1/auth/forgot-password", [auth_manager](const httplib::Request& req, httplib::Response& res) {
        try {
            json body = json::parse(req.body);
    ...
    // ========================================================================
    server.Post("/api/v1/auth/reset-password", [auth_manager](const httplib::Request& req, httplib::Response& res) {
        try {
            json body = json::parse(req.body);
    ...
    server.Post("/api/v1/auth/apikeys", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require authentication
        auto user = auth_middleware->getRequestUser(req);
        if (!user) {
            sendError(res, 401, "Authentication required");
        ...
        try {
            ...
            std::vector<std::string> scopes = body.value("scopes", std::vector<std::string>{"read"});
    ...
    server.Get("/api/v1/auth/apikeys", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require authentication
        auto user = auth_middleware->getRequestUser(req);
        if (!user) {
            sendError(res, 401, "Authentication required");
        ...
        for (const auto& key : keys) {
            keys_json.push_back({
                ...
                {"scopes", key.scopes},
                {"created_at", key.created_at.seconds},
                {"expires_at", key.expires_at.seconds},
    ...
    server.Delete(R"(/api/v1/auth/apikeys/([^/]+))", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require authentication
        auto user = auth_middleware->getRequestUser(req);
        if (!user) {
            sendError(res, 401, "Authentication required");
    ...
    server.Get("/api/v1/users", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require admin role
    ...
    server.Post("/api/v1/users", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require admin role
        if (!auth_middleware->requireRole(req, res, "admin")) {
            return;
        }
    ...
    server.Put(R"(/api/v1/users/([^/]+))", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require admin role
        if (!auth_middleware->requireRole(req, res, "admin")) {
            return;
        }
    ...
    server.Delete(R"(/api/v1/users/([^/]+))", [auth_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
        // Require admin role
        if (!auth_middleware->requireRole(req, res, "admin")) {
            return;
        }
