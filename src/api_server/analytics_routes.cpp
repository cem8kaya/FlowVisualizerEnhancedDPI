...
void setupAnalyticsRoutes(httplib::Server& server, AnalyticsManager* analytics_manager,
    ...
    server.Get(
        ...
        [analytics_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
            ...
            try {
                ...
                json response = {{"total_jobs", summary.total_jobs},
        ...
        [analytics_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
            ...
            try {
                ...
                for (const auto& s : stats) {
                    stats_json.push_back({{"protocol", s.protocol},
                ...
                json response = {{"protocols", stats_json}};
        ...
        [analytics_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
            ...
            try {
                ...
                for (const auto& t : talkers) {
                    talkers_json.push_back({{"ip_address", t.ip_address},
                                            {"packet_count", t.packet_count},
                                            {"byte_count", t.byte_count},
                                            {"session_count", t.session_count}});
                }

                json response = {{"top_talkers", talkers_json}, {"limit", limit}};
    ...
    server.Get("/api/v1/analytics/performance", [analytics_manager, auth_middleware](
        ...
        try {
            ...
            json response = {{"avg_parsing_throughput_mbps", metrics.avg_parsing_throughput_mbps},
    ...
    server.Get(
        ...
        [analytics_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
            ...
            try {
                ...
                std::string interval =
                    req.has_param("interval") ? req.get_param_value("interval") : "1h";
                std::string metric =
                    req.has_param("metric") ? req.get_param_value("metric") : "jobs";
                ...
                for (const auto& p : points) {
                    points_json.push_back({{"timestamp", p.timestamp}, {"value", p.value}});
                }

                json response = {{"metric", metric}, {"interval", interval}, {"data", points_json}};
    ...
    server.Post(
        ...
        [analytics_manager, auth_middleware](const httplib::Request& req, httplib::Response& res) {
            ...
            json response = {{"message", "Analytics cache cleared successfully"}};
