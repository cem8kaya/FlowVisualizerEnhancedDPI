...
void PacketProcessor::processTransportAndPayload(const PacketMetadata& metadata,
    ...
    if ((metadata.five_tuple.protocol == IPPROTO_TCP ||
        ...
        if (ProtocolDetector::isSipPayload(payload.data(), payload.size())) {
            LOG_DEBUG("Content-based SIP detection: port "
                      << metadata.five_tuple.src_port << "->" << metadata.five_tuple.dst_port);
    ...
    if (metadata.five_tuple.protocol == IPPROTO_UDP &&
        ...
        if (msg.has_value()) {
            ...
            if (!msg->user_data.empty() && msg->user_data.size() >= 20) {
                ...
                inner_metadata.gtp_teid = msg->teid;
                ...
                if (ip_version == 4 && inner_len >= 20) {
                    ...
                    if (inner_len > ip_hdr_len + 4) {
                        ...
                        if (iph->ip_p == IPPROTO_TCP || iph->ip_p == IPPROTO_UDP) {
                            inner_metadata.five_tuple.src_port = ntohs(*reinterpret_cast<const uint16_t*>(trans));
                            inner_metadata.five_tuple.dst_port = ntohs(*reinterpret_cast<const uint16_t*>(trans + 2));
                        }
                        valid_inner = true;
                    }
                ...
                if (valid_inner) {
                    LOG_DEBUG("GTP-U inner packet: TEID=" << msg->teid
                              << " inner 5-tuple: " << inner_metadata.five_tuple.src_ip
...
bool PacketProcessor::isSipPort(uint16_t port) {
    ...
    return port == 5060 || port == 5061 || port == 5062 || port == 5063 ||
           port == 5064 || port == 6101 || port == 7100 || port == 7200;
}
...
PacketDeduplicator::PacketDeduplicator(size_t window_size)
    : max_window_size_(window_size) {
    LOG_DEBUG("PacketDeduplicator initialized with window_size=" << window_size);
}

bool PacketDeduplicator::isDuplicate(const PacketMetadata& meta, const std::vector<uint8_t>& payload) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (!payload.empty()) {
        if (meta.five_tuple.protocol == IPPROTO_TCP && payload.size() >= 4) {
            ...
            sig.seq_or_id = (payload[0] << 24) | (payload[1] << 16) |
    ...
    for (const auto& existing : recent_packets_) {
        ...
        if (existing.src_ip_hash == sig.src_ip_hash &&
            existing.dst_ip_hash == sig.dst_ip_hash &&
            existing.src_port == sig.src_port &&
            existing.dst_port == sig.dst_port &&
            existing.protocol == sig.protocol &&
            existing.payload_len == sig.payload_len &&
            existing.payload_crc == sig.payload_crc &&
            existing.seq_or_id == sig.seq_or_id) {
...
bool PacketDeduplicator::PacketSignature::operator==(const PacketSignature& other) const {
    return src_ip_hash == other.src_ip_hash &&
           dst_ip_hash == other.dst_ip_hash &&
           src_port == other.src_port &&
           dst_port == other.dst_port &&
           protocol == other.protocol &&
           seq_or_id == other.seq_or_id &&
