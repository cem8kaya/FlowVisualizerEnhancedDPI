...
private:
     ...
     */
    bool parseSecurityHeader(const uint8_t* data, size_t len,
                            LteNasMessage& msg, size_t& offset);

    /**
     * Parse plain NAS message (no security)
     */
    bool parsePlainMessage(const uint8_t* data, size_t len,
                          LteNasMessage& msg);

    /**
     * Parse EMM message
     */
    bool parseEmmMessage(const uint8_t* data, size_t len,
                        size_t offset, LteNasMessage& msg);

    /**
     * Parse ESM message
     */
    bool parseEsmMessage(const uint8_t* data, size_t len,
                        size_t offset, LteNasMessage& msg);

    /**
     * Extract IMSI from EPS Mobile Identity IE
     */
    std::optional<std::string> extractImsi(const uint8_t* data, size_t len);

    /**
     * Extract GUTI from EPS Mobile Identity IE
     */
    std::optional<std::string> extractGuti(const uint8_t* data, size_t len);

    /**
     * Decode APN from NAS IE
     */
